...
const ClassesFriday = ({close}) => {
    ...
    useEffect(() => { 
        axios.get("http://localhost:4000/getAllClasses")
            .then((res) => { 
              const docs = res.data
              const classesOnFriday = docs.filter(d => d.day === "Friday")
              setFridayClasses(classesOnFriday)
              setTimeout(() =>{ 
                setLoad(false)
                console.log("fALSO")
              }, 1200)
  ...
  return (
    <div>
               ...
               <div className='flex flex-row flex-wrap ml-12'>
                   {fridayClasses.map((c) => <StructureClasses monday={c}/>)} 
                </div>    
                </>
