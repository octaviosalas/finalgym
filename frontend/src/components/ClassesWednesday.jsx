...
const ClassesWednesday = ({close}) => {
    ...
    useEffect(() => { 
        axios.get("http://localhost:4000/getAllClasses")
            .then((res) => { 
              const docs = res.data
              const classesOnWednesday = docs.filter(d => d.day === "Wednesday")
              setWednesdayClasses(classesOnWednesday)
              setTimeout(() =>{ 
                setLoad(false)
                console.log("fALSO")
              }, 1200)
  ...
  return (
    <div>
               ...
               <div className='flex flex-row flex-wrap ml-12'>
                   {wednesdayClasses.map((c) => <StructureClasses monday={c}/>)} 
                </div>    
                </>
...
export default ClassesWednesday
